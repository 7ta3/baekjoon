...
def solution(width, height, diagonals):
    ...
    while ST:
        x, y, distance, visited, visited_diagonal = ST.pop()

        if distance > min_val : continue
        ...
        for i in G[y][x]:
            if not visited[i[1]][i[0]]:
                if i[-1] == 1 :
                    ...
                    ST.append([i[0], i[1], distance + i[-1], visited, visited_diagonal])
                    visited[i[1]][i[0]] = False
                elif not visited_diagonal and i[-1] != 1:
                    visited[i[1]][i[0]] = True
                    ST.append([i[0], i[1], distance + i[-1], visited, True])
                    visited[i[1]][i[0]] = False
    print(answer)
    return answer
